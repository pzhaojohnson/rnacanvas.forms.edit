...
import * as styles from './TertiaryBondsAddTools.module.css';
...
export class TertiaryBondsAddTools {
  readonly #targetApp;
  ...
  readonly #onlyAddMissingField = new CheckboxField('Only add missing tertiary bonds', this.#onlyAddMissingCheckbox.domNode);
  ...
  constructor(targetApp: App) {
    ...
    this.domNode.classList.add(styles['tertiary-bonds-add-tools']);
    ...
    // watch for any tertiary bonds being added or removed from the drawing
  ...
  press(): void {
    ...
    let existingPairs = allTertiaryBonds.map(wrap).map(sb => sb.basePair);
    ...
    if (this.#onlyAddMissingCheckbox.domNode.checked) {
      missingPairs.forEach(pair => this.#targetApp.drawing.addTertiaryBond(...pair));
    } else {
      antiParallelPairs(selectedBases).forEach(pair => this.#targetApp.drawing.addTertiaryBond(...pair));
    }
  }
